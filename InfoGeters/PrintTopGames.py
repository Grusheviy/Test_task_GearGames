...
class PrintTopGames:
    ...
    def print_top_games_info(self):
        try:
            ...
            top_games_searcher = TopGamesSearcher()

            # Блок 1: Поиск Art Of War 3 среди результатов запроса "best strategy game"
            try:
                top_games = top_games_searcher.search_top_games('best strategy game', n_hits=3)
                ...
                if art_of_war_3_in_results:
                    ...
                    art_of_war_3_info = app_info.get_app_info(top_games[0]['appId'])
                    ...
                    if user_input_info.lower() == 'да':
                        print_review(app_reviews.get_app_reviews(top_games[0]['appId'], count=3))
                ...
                # Если "Art Of War 3" не найдена в результате запроса
                    ...
                    for i, game in enumerate(top_games, start=1):
                        ...
                        game_info = app_info.get_app_info(game['appId'])
                        ...
                        if user_input_review.lower() == 'да':
                            # Получение отзывов
                            game_reviews = app_reviews.get_app_reviews(game['appId'], count=5)
